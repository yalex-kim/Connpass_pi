...
- **DB**: SQLite
- **백엔드**: Python (Flask) — RAG/Jira/MCP/Skill API
...
[Node.js 서버]  ← pi-agent-core Agent loop
    ↕ HTTP
[Flask app.py]  ← RAG / Jira / MCP / Skill / SQLite API
    ↕
[사내 vLLM / RAGaaS / Gerrit / Jira]
```
...
### Flask(Python)이 담당하는 것 (REST API로 노출)
- RAG 검색 (`/api/rag/search`)
- Jira 조회/업데이트 (`/api/jira/*`)
- MCP 서버 연동 (`/api/mcp/call`)
- Skill CRUD (`/api/skills`)
- 임시 RAG 파일 업로드 (`/api/sessions/{id}/upload`)
- SQLite DB 관리
...
- MCP 서버 등록/연결 (SSE)
- Session 저장 + Stop 버튼
- emitStatus await 확인 (tool call 상태 표시 버그)

### 다음
- 세션 내 임시 RAG (파일 업로드 + search_document tool)
- Index 메타데이터 관리
- Agent 지능적 index 선택

### 이후
- Jira 이슈 배치 구조화 (RCA Map 구축)
- 로그 기반 유사 이슈 검색 Skill
- Skill Creator
